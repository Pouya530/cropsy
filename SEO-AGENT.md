...
## 14. Cropsy (`cropsy.app`) — live static stack (April 2026)

This repo is **not** Next.js: the marketing homepage and routing are served by **`index.js`** (Node HTTP + Vercel serverless `api/index.js`). Public simulations are **static HTML** under `public/`. When applying Sections 1–6 above, translate patterns into:

| Concern | Implementation |
|--------|----------------|
| **Canonical & OG on homepage** | `metaTagsBlock()` + `jsonLdWelcomeBundle()` in `index.js`; uses `SITE_ORIGIN` in production for correct absolute URLs. |
| **Production origin** | Set **`SITE_ORIGIN=https://www.cropsy.app`** (or your chosen canonical host) on Vercel so canonical and Open Graph URLs match the live domain. |
| **Default OG image** | `/og-cropsy-default.png` (also `public/og-cropsy-default.png`). |
| **Analytics** | **Google Analytics 4** — measurement ID **`G-H0JB9SJT0R`**. The gtag.js snippet (async loader + `gtag('config', ...)`) is in **`<head>`** on: `index.js` (homepage HTML), `public/grass-crop-circle.html`, `public/field.html`, `public/crop-circle-3d.html`, `public/demo/index.html`, `public/hero-field-embed.html`, and root `crop-circle-3d.html`. Update this table if new top-level HTML pages are added. |
| **Key public URLs** | `/` (home), `/grass-crop-circle.html`, `/field.html`, `/crop-circle-3d.html`, `/demo/`, `/hero-field-embed.html` (embed). |

Simulation pages: ensure each has a unique `<title>` and, where relevant, `<meta name="description">` (see `public/field.html` as an example).
